...
resource "aci_rest_managed" "spanRsDestPathEp_port" {
  count      = var.node_id != 0 && var.channel == "" && var.sub_port == 0 && var.port != 0 ? 1 : 0
  dn         = "${aci_rest_managed.spanDest.id}/rsdestPathEp-[${format("topology/pod-%s/paths-%s/pathep-[eth%s/%s]", var.pod_id, var.node_id, var.module, var.port)}]"
...
resource "aci_rest_managed" "spanRsDestPathEp_subport" {
  count      = var.node_id != 0 && var.channel == "" && var.sub_port != 0 && var.port != 0 ? 1 : 0
  dn         = "${aci_rest_managed.spanDest.id}/rsdestPathEp-[${format("topology/pod-%s/paths-%s/pathep-[eth%s/%s/%s]", var.pod_id, var.node_id, var.module, var.port, var.sub_port)}]"
  class_name = "spanRsDestPathEp"
  content = {
    mtu = var.mtu
...
resource "aci_rest_managed" "spanRsDestPathEp_channel" {
  count      = var.node_id != 0 && var.channel != "" && var.sub_port == 0 && var.port == 0 ? 1 : 0
  dn         = "${aci_rest_managed.spanDest.id}/rsdestPathEp-[${format("topology/pod-%s/paths-%s/pathep-[%s]", var.pod_id, var.node_id, var.channel)}]"
  class_name = "spanRsDestPathEp"
  content = {
    mtu = var.mtu
...
resource "aci_rest_managed" "spanRsDestEpg" {
  count      = var.tenant != "" && var.application_profile != "" && var.endpoint_group != "" && var.ip != "" && var.source_prefix != "" ? 1 : 0
